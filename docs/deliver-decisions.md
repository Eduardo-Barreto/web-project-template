...
**Semgrep roda só no CI, em container.** Metade do `p/owasp-top-ten` procura sinks que um SPA client-only não tem, e o que sobra o oxlint já pega, então ele não entra no loop local. O `semgrep-action` está deprecado; o container `semgrep/semgrep` é o caminho suportado e dispensa instalar Python no runner.
