...
- `bun run doctor`: oxlint (type-aware plus the project's own rules), oxfmt check, typecheck, knip, unit tests. Runs in about thirteen seconds, most of it `scripts/init.test.ts` running the whole gate again inside a fresh project. Run before every commit.
